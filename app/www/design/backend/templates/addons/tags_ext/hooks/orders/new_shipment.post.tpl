    ...
    <div class="control-group {if $allow_save}cm-no-hide-input{/if}">
        ...
        <div class="controls">
            <ul id="my_tags">
                <textarea name="my_tags" type="text" data-ca-tags="myTagsValues" class="hidden-accessible">{foreach $order_info.tags as $tag}{$tag.tag}{if !$tag@last}
                    {/if}{/foreach}</textarea>
                ...
                {foreach from=$order_info.tags item="tag" name="tag"}<li>{$tag.tag}</li>{/foreach}
            </ul>
        </div>
    </div>
